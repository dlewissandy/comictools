...
- **Splitting breadcrumb** through the creative hierarchy (Studio › House › Series › Issue › Panel); the first crumb is a split control offering the studio's two rooms — the **Studio wall** (make) and the **Reading Room** (read)
...
- **One conversation** — a single dialog follows you across every room (walks leave a slim MEANWHILE caption); what *happened* lives in the daybook, and one Editor with every tool answers everywhere
...
- **A studio, not a chatbot.** Assets — characters, looks, wardrobes, settings, props, styles — lay onto the light table from the bench's own pickers (each with a 'Borrow from another series…' door), the Reading Room reads every house's books, and Cmd-K jumps anywhere. Batch renders run in the background, filing receipts in the daybook while you keep talking, with a running spend estimate in the header.
