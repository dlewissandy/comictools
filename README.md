...
- **A studio, not a chatbot.** Assets — characters, variants, outfits, settings, props, styles — live in a catalog drawer summonable from any board view: filter, search, click a tile and it lands straight on the light table. Batch renders run in the background, posting each finished panel into the chat while you keep talking, with a running spend estimate in the header.
